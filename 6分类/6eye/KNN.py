...
# 训练KNN模型
knn.fit(X_train_s_res, y_train_res)

# 预测测试集
y_pred = knn.predict(X_test_s)
y_pred_proba = knn.predict_proba(X_test_s)
...
# 计算ROC曲线和AUC
y_test_one_knn = label_binarize(y_test, classes=np.unique(y_train))
knn_AUC = {}
knn_FPR = {}
knn_TPR = {}

for i in range(y_test_one_knn.shape[1]):
    knn_FPR[i], knn_TPR[i], _ = roc_curve(y_test_one_knn[:, i], y_pred_proba[:, i])
    knn_AUC[i] = auc(knn_FPR[i], knn_TPR[i])
print("ROC AUC for each class:", knn_AUC)

# 计算宏平均ROC曲线和AUC
knn_FPR_final = np.unique(np.concatenate([knn_FPR[i] for i in range(y_test_one_knn.shape[1])]))
knn_TPR_all = np.zeros_like(knn_FPR_final)
for i in range(y_test_one_knn.shape[1]):
    knn_TPR_all += np.interp(knn_FPR_final, knn_FPR[i], knn_TPR[i])
knn_TPR_final = knn_TPR_all / y_test_one_knn.shape[1]
knn_AUC_final = auc(knn_FPR_final, knn_TPR_final)
print(f"Macro Average AUC with KNN: {knn_AUC_final}")

# 绘制ROC曲线
plt.figure(figsize=(10, 5), dpi=300)
for i, color in zip(range(y_test_one_knn.shape[1]), ['b', 'g', 'r', 'c', 'm','y']):
    plt.plot(knn_FPR[i], knn_TPR[i], color=color, linestyle='-', label=f'Class {i} ROC AUC={knn_AUC[i]:.4f}', lw=0.8)
plt.plot(knn_FPR_final, knn_TPR_final, color='black', linestyle='-', label=f'Macro Avg ROC AUC={knn_AUC_final:.4f}', lw=1)
...
for i in range(y_test_one_knn.shape[1]):
    pr_Recall[i], pr_Precision[i], _ = precision_recall_curve(y_test_one_knn[:, i], y_pred_proba[:, i])
    pr_AUC[i] = average_precision_score(y_test_one_knn[:, i], y_pred_proba[:, i])
