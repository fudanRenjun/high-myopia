...
# 加载训练集和测试集数据
train_data = pd.read_csv('E:/RS/6eye/feature/RF/train-80-1_processed.csv')  # 训练集路径
test_data = pd.read_csv('E:/RS/6eye/feature/RF/validation-20-1_processed.csv')    # 测试集路径
...
# 构建随机森林模型，并使用class_weight='balanced'（另一种平衡类别的方式）
model_rf = RandomForestClassifier(n_estimators=200, random_state=42)
model_rf.fit(X_train_s_res, y_train_res)
