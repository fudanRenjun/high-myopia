...
print(f"Macro Average AUC with AdaBoost: {rf_AUC_final}")

# 绘制ROC曲线
plt.figure(figsize=(10, 5), dpi=300)
for i in range(ytest_one_rf.shape[1]):
    plt.plot(rf_FPR[i], rf_TPR[i], linestyle='-', label=f'Class {i+1} ROC AUC={rf_AUC[i]:.4f}', lw=0.8)
plt.plot(rf_FPR_final, rf_TPR_final, color='black', linestyle='-', label=f'Macro Average ROC AUC={rf_AUC_final:.4f}', lw=1)
...
plt.figure(figsize=(10, 5), dpi=300)
for i in range(ytest_one_rf.shape[1]):
    plt.plot(pr_Recall[i], pr_Precision[i], linestyle='-', label=f'Class {i+1} PR AUC={pr_AUC[i]:.4f}', lw=0.8)
